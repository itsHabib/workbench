...
| **P3 — the policy** | `require_at_tier` decodes, is honoured, fails closed. | `Declaration.RequireAtTier`; **add `require_at_tier` to `review-panel-v1.json`'s `declaration.properties`** or `additionalProperties:false` rejects every artifact the feature makes; **extend the `.ship.json` nesting wrapper and keep translating** — do *not* parse `Declaration` directly (§5); **add Go↔schema parity coverage** — nothing today catches an omitted schema property; **[PENDING §10.7 — do not start until §4.2 is settled] make `reviewVerdictIDs`' `-reviews-optional` early return ([`main.go:551-553`](../../../cmd/gate/main.go)) conditional on the panel having been *required*** — §4.2's recommended option is a caller-side change, not just a policy statement, and without it CI keeps hard-skipping; §7.1 decision flow; §8 table as a table test; verdict `why` wording. | P2 | ~220 LOC |
