...
1. **It removes the parks it targets.** The §9 offline analysis over `log.jsonl`, run at threshold `"T1"`: every recorded run whose *only* escalation reason was `review-panel-completeness` at floor tier T0 now evaluates to pass. Target: that class goes to zero. (Not `gate backtest` — §9.)
