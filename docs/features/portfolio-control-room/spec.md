...
Dossier breaker accounting is per refresh cycle: one cycle contributes at most one consecutive failure regardless of which lifecycle stage fails, and there is never a second start attempt in that cycle. While open, automatic collection returns an immediate `unavailable/breaker_open` receipt. Exactly one manual half-open probe may run; concurrent manual callers join that probe and receive the same result, while automatic callers remain suppressed. Child cleanup closes stdin, waits up to one second, then kills and waits/reaps if needed. A successful first call resets the counter.
