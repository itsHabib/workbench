...
| `pr.merge_ready` | actionable | 65 | GitHub receipt is current; `detail_state == complete`; PR is non-draft; at least one visible check exists and every visible check completed successfully; `reviewDecision == APPROVED`; `mergeable == MERGEABLE`; unresolved thread count is zero. Empty or unknown never qualifies. |
