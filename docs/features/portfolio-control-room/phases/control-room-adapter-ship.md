...
- Execute only enriched `ship list --json`, targeted `ship status <workflow-id> --json`, `ship driver list --json`, and point `ship driver status <driver-id> --json` when inventory data requires detail.
