...
**Status:** built (P0 — read-only docket) — `cmd/console/` (landed 2026-07-20, 3 test files). Design record, not an open proposal. P1/P2 (the action security plane and the browser judgment form) are not built: every registered route is a `GET`, and judging stays in the CLI. Browser-based minting remains a no-go under the current threat model.
**Owner:** @itsHabib
**Date:** 2026-07-21
...
- **Console restart after commit but before response:** refresh the case file. Gate's ledger is canonical; a new attempt against the resolved escalation returns `stale_run`, while the recorded result remains visible. If the same crash also interrupted the anchor bind, the intent that could retry is gone with the process — recovery then runs ledger-derived, not memory-derived: see the restart path in §8.7.
...
- Gate tests for request-id idempotency (checked before the stale-escalation precondition), partial-write resume, the no-append anchor reconcile on terminal-result replay, ledger-derived recovery after console restart (no in-memory intent; request id recovered from the tail artifact), two-request same-escalation race, stale escalation, stale/expired/wrong-scope grant, and audit tamper with zero unauthorized append.
...
3. Reduce terminal state by PR subject, not only run ID. For all runs referring to the same `repo#number`, the newest terminal artifact wins: newest terminal is an escalation → exactly one actionable parked row; newest terminal is an action → no actionable row for that PR. A later judged/released run supersedes earlier parked attempts. **"Newest" means latest position in the append-only log — chain order, not artifact timestamps** (wall-clocks can skew between writers; the hash chain's order is the one gate authenticates). Ties cannot exist: the log is a total order.
...
6. Add an explicit live-read mode: it may query GitHub read-only to retain OPEN PRs and drop MERGED/CLOSED. One lookup per distinct `repo#number` in the projection (bounded by docket size, never per run), so a large ledger cannot amplify into a rate-limit burn; a rate-limited or failed lookup fails visible and keeps the row as `unknown` (never silently dropped). The default non-live projection stays deterministic and offline.
