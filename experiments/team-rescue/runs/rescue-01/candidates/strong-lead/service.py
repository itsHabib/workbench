...
def _valid_state(state):
    ...
    for key, delivery in deliveries.items():
        if not isinstance(key, str) or not isinstance(delivery, dict):
            return False
        if set(delivery) != required:
            return False
        if any(not isinstance(delivery[field], str)
               for field in ("id", "event_id", "subscription_id", "url")):
            return False
        if delivery["status"] not in statuses:
            return False
        if type(delivery["attempts"]) is not int or delivery["attempts"] < 0:
            return False
        next_at = delivery["next_attempt_at"]
        if next_at is not None and type(next_at) is not int:
            return False
        ...
        for entry in delivery["history"]:
            if not isinstance(entry, dict):
                return False
            if set(entry) != {"attempt", "at", "status_code"}:
                return False
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_error(self, code, message=None, explain=None):
        self.reply(code if 400 <= code < 500 else 400, {
    ...
    def mutate(self, operation):
        ...
        try:
            ...
            SUBSCRIPTIONS.clear()
            SUBSCRIPTIONS.update(snapshot[0])
            EVENTS.clear()
            EVENTS.update(snapshot[1])
            DELIVERIES.clear()
            DELIVERIES.update(snapshot[2])
            raise
                    ...
                    def create():
                        ...
                        for subscription_id, url in SUBSCRIPTIONS.items():
                            ...
                            DELIVERIES[delivery_id] = {
                                ...
                                "payload": body["payload"], "status": "pending",
                                "attempts": 0, "next_attempt_at": 0, "history": [],
                            }
                    ...
                    def tick():
                        ...
                        for delivery in DELIVERIES.values():
                            ...
                            delivery["history"].append({
                                "attempt": delivery["attempts"], "at": now,
                                "status_code": status,
                            })
    ...
    def do_GET(self):
        with STATE_LOCK:
            ...
            if self.path == "/metrics":
                ...
                self.reply(200, {
                    "subscriptions": len(SUBSCRIPTIONS), "events": len(EVENTS),
                    "deliveries": len(DELIVERIES), **statuses,
