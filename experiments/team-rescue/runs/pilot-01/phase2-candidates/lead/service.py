...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            replay_prefix = "/deliveries/"
            replay_suffix = "/replay"
            if self.path.startswith(replay_prefix) and self.path.endswith(replay_suffix):
                delivery_id = self.path[len(replay_prefix):-len(replay_suffix)]
                ...
                with DB_LOCK:
                    ...
                    if row["status"] not in ("succeeded", "failed"):
                        self.reply(409, {"error": "delivery is not terminal"})
                        return
                    DB.execute(
