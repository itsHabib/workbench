...
func TestDecideDoesNotAddressFromClosedUnchangedFinding(t *testing.T) {
	plan := routedPlan(t, "T1")
	input := cleanInput(plan, 1)
	input.Findings = []reviewroute.FindingState{
		{
			ID: "closed", Severity: "low", Reviewers: []string{"codex"},
			Disposition: "fixed", Changed: false, ReviewerClosed: true,
		},
		{
			ID: "changed", Severity: "medium", Reviewers: []string{"codex"},
			Disposition: "fixed", Changed: true, ReviewerClosed: false,
		},
	}
	decision, err := Decide(plan, input, testNow)
...
func TestDecideParksAcceptedFindingAtCycleCap(t *testing.T) {
	plan := routedPlan(t, "T1")
	input := cleanInput(plan, plan.MaxCycles)
	input.Findings = []reviewroute.FindingState{{
		ID: "codex-1", Severity: "medium", Reviewers: []string{"codex"},
		Disposition: "fixed", Changed: false, ReviewerClosed: false,
	}}
