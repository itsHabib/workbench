...
recipe `swarm` supports today, in the order the work happens.
...
It does not tick. `swarm poc init --tasks N --packages P` is a stand-in that generates both from
...
```sh
swarm init --operator <owner> --lead <any name that may rule above peers, or none>
swarm install-hook
swarm watch --fetch --wake --wake-model <model> --verify "go test ./..." --disk-min 10G
```
...
A seat starts when `swarm admit --seats N --disk-min` says so. Seats are branches in their own
...
- Contention: `swarm check` before an edit, at package scope. Contended and unruled means ask.
- Rulings: `swarm ask` routes by affinity to the seats with context; they rule with
  `swarm rule --order`; ties resolve by tier. The ledger is the "decided" section.
...
`swarm order` prints the merge order from the order rulings and start times. A consolidator
...
`swarm load` shows, per address, how long requests sit before someone claims them. When the
