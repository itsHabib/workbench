...
supersedes. Effective rulings are those no later ruling names. `swarm check <path>` and
`swarm decisions --scope` are the "decided" section of the old board, as a query.
...
**Routing.** `swarm ask` fans out to seats with affinity on the scope: branches whose changed
...
**Resources.** `swarm take` and `swarm drop` lease an exclusive thing to a seat with an epoch
...
binary's path as `install-hook` writes it, `swarm.exe hook` on Windows.
