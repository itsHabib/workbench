...
func (s *State) Ask(from string, scope []string, question string, options []string, needs string) (*Request, error) {
	if from == "" {
		return nil, refuse("no_from", "who is asking? pass --as <seat> or set SWARM_SEAT")
	}
...
func (s *State) route(r *Request) {
	...
	text := fmt.Sprintf("request %s from %s on %s needs %s: %q. rule it if the evidence is yours: `swarm rule %s --ruling '...' --evidence '...'`; escalate if it needs intent: `swarm escalate %s --to operator --why '...'`.", r.ID, r.From, strings.Join(r.Scope, ","), r.Needs, r.Question, r.ID, r.ID)
