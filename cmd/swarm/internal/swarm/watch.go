...
func rowAlerts(b *Board, as *alertSet) {
	for _, r := range b.Rows {
		...
		case "silent":
			as.add(Alert{Key: "silent:" + r.Branch, Kind: "builder_silent", Subject: r.Branch, Seats: []string{r.Branch},
				Text: fmt.Sprintf("your branch %s has not moved for %s. push WIP now, or write RESULT.json if you are done, or `swarm ask` if you are blocked.", r.Branch, ageString(r.AgeSeconds))})
		case "pin_violation":
	...
	for _, c := range b.Contended {
		...
		as.add(Alert{Key: "overlap:" + c.File, Kind: "overlap_unruled", Subject: c.File, Seats: c.Branches,
			Text: fmt.Sprintf("%s is changed by %s with no ruling on order. one of you: `swarm ask --scope %s --question 'which lands first?' --options '<a>|<b>'`, then `swarm wait`.", c.File, strings.Join(c.Branches, " and "), c.File)})
	}
}
	...
	for _, r := range reqs {
		...
		if r.Status == "claimed" && r.Claim != nil && as.now.After(r.Claim.Until) {
			as.add(Alert{Key: "staleclaim:" + r.ID, Kind: "claim_stale", Subject: r.ID, Seats: []string{r.Claim.Holder},
				Text: fmt.Sprintf("your claim on %s expired at %s without a ruling. rule it (`swarm rule %s ...`) or let it go.", r.ID, r.Claim.Until.Format("15:04"), r.ID)})
		}
		...
		as.add(Alert{Key: "unclaimed:" + r.ID, Kind: "request_unclaimed", Subject: r.ID, Seats: audience(r, t, working),
			Text: fmt.Sprintf("request %s from %s has waited %s (needs %s): %q on %s. if it is in your reach: `swarm rule %s --ruling '...' --evidence '...'`; if it needs product intent: `swarm escalate %s --to operator --why '...'`.", r.ID, r.From, ageString(int64(age.Seconds())), r.Needs, r.Question, strings.Join(r.Scope, ","), r.ID, r.ID)})
	}
}
	...
	for _, res := range b.Resources {
		if as.now.After(res.Until) {
			as.add(Alert{Key: "resource:" + res.Name, Kind: "resource_expired", Subject: res.Name, Seats: []string{res.Holder},
				Text: fmt.Sprintf("your lease on %s expired at %s. `swarm take %s` to renew or `swarm drop %s`.", res.Name, res.Until.Format("15:04"), res.Name, res.Name)})
		}
	}
}
