...
swarm split --into "t%02db-%s-k%db:add key k%db=%d0 to pkg/%s Values() with a test|t%02dc-%s-k%dc:add key k%dc=%d00 to pkg/%s Values() with a test" --why "scoped wrong: three keys"
...
func LoadTasks(main string) ([]Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s is not a sandbox from `swarm poc init` (%w)", main, err)
	}
	var tasks []Task
...
1. One worktree, one branch: the ones you were given. Never edit outside your worktree.
   ...
   swarm split --into "<branch>:<title>|<branch>:<title>" --why "..."
2. Commit and push WIP within 5 minutes of starting and every 10 minutes after:
   git push origin <your branch>
3. Before editing any file, run: swarm check <path>
   ...
   swarm ask --scope <package dir> --question "..." --options "a|b"
   then: swarm wait <id>
   Follow the ruling. Rebase on the other branch if the ruling says it lands first.
   When you rule on order, record it: swarm rule <id> --order "<first>,<second>" --ruling "..." --evidence "..."
   Your card's files are already declared as your intent. If you find you must change a path
   your card did not list, declare it first: swarm intend <path>
4. To change anything under fixtures/, hold the resource first: swarm take fixture-db
   If refused, someone holds it: retry every minute for up to 8 minutes. When done: swarm drop fixture-db
5. If the task is ambiguous about product behavior, never guess:
   swarm ask --needs operator --scope <path> --question "..."
   then: swarm wait <id> --timeout 8m
...
8. If swarm wait times out: push WIP, write RESULT.json with "blocked": "<request id>" in place of
   claims, commit it, push, and stop.
9. One shell command per tool call. No && and no ; between commands.
10. Notes addressed to you arrive as [swarm ...] lines; swarm inbox shows them too. Act on them.
`
...
`,
  ...
  "hooks": {
    "SessionStart": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}],
    "UserPromptSubmit": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}],
    "PreToolUse": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}],
    "PostToolUse": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}],
    "Stop": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}],
    "SessionEnd": [{"matcher": "", "hooks": [{"type": "command", "command": "swarm hook"}]}]
  }
}
`,
}
...
func Init(dir string, spec Spec) error {
	...
	if _, err := swarm.Git(dir, "init", "--bare", "--initial-branch=main", origin); err != nil {
		return err
	}
	if _, err := swarm.Git(dir, "init", "--initial-branch=main", main); err != nil {
		return err
	}
	for _, kv := range [][2]string{
		...
		if _, err := swarm.Git(main, "config", kv[0], kv[1]); err != nil {
			return err
		}
	}
	files := map[string]string{}
	...
	if _, err := swarm.Git(main, "add", "-A"); err != nil {
		return err
	}
	if _, err := swarm.Git(main, "commit", "-q", "-m", "sandbox base"); err != nil {
		return err
	}
	if _, err := swarm.Git(main, "remote", "add", "origin", origin); err != nil {
		return err
	}
	if _, err := swarm.Git(main, "push", "-q", "-u", "origin", "main"); err != nil {
		return err
	}
	...
	if err := s.Init(swarm.Tiers{Operator: []string{"operator"}, Lead: []string{"lead"}}); err != nil {
		return err
	}
