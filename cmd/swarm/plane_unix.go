...
const planeUsage = `swarm plane: the coordination store and its fault harness
  ...
  swarm plane worker JSON        (started by the harness)
`
...
func (r *redisProc) start() error {
	r.cmd = exec.Command("redis-server", "--port", fmt.Sprint(r.port), "--bind", "127.0.0.1", "--appendonly", "yes",
		"--appendfsync", "always", "--dir", r.dir, "--save", "")
	if err := r.cmd.Start(); err != nil {
		return err
	}
	for i := 0; i < 100; i++ {
		if s, err := plane.OpenRESP(fmt.Sprintf("127.0.0.1:%d", r.port), "ping:"); err == nil {
			_ = s.Close()
			return nil
		}
...
func planeFault(args []string) int {
	...
	failed := 0
	fmt.Printf("%-5s %-5s %-7s %-6s %-6s %-6s %-8s %-7s %-9s %-10s %-9s %s\n", "seed", "store", "accept", "tasks", "events", "crash", "refused", "taken", "recoverMS", "violations", "unfinish", "verdict")
	for seed := 1; seed <= *seeds; seed++ {
		...
		fmt.Printf("%-5d %-5s %-7d %-6d %-6d %-6d %-8d %-7d %-9d %-10d %-9d %s\n", rep.Seed, rep.Store, rep.Check.Accepted, rep.Tasks, rep.Events,
			rep.Crashed, rep.Fenced, rep.Takeovers, rep.MaxRecoverMS, len(rep.Check.Violations), len(rep.Check.Unfinished), verdict)
