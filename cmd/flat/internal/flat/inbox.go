...
// SeatOf resolves the seat a session is: FLAT_SEAT, else the branch checked
// out at cwd. Location is identity; a renamed session changes nothing.
func SeatOf(cwd string) string {
	if v := os.Getenv("FLAT_SEAT"); v != "" {
		return v
	}
...
func Hook(in io.Reader, out io.Writer) error {
	...
	if ev.Event == "SessionStart" {
		...
		lines = append(lines, fmt.Sprintf("[flat] you are seat %s. %d decision requests open (%s). `flat inbox` reads your notes; `flat board --md` reads the fleet.", seat, summary.Open, summary.byNeeds()))
		if ev.SessionID != "" {
	...
	for _, n := range notes {
		lines = append(lines, fmt.Sprintf("[flat %s] %s", n.Kind, n.Text))
	}
	if len(lines) == 0 {
		return nil
	}
...
// project. cmd is the flat binary path (flat.exe on Windows).
