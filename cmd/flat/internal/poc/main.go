...
const usage = `flat poc: the adversarial proof of concept
  ...
  flat poc stats --dir DIR --run RUNDIR [--json]
  flat poc compare --flat RUNDIR --tree RUNDIR
`
...
func runCmd(args []string) error {
	...
	var err error
	if o.DiskMin, err = flat.ParseBytes(diskMin); err != nil {
		return err
	}
	return Run(o)
}
...
func compareCmd(args []string) error {
	...
	load := func(run string) (*Score, error) {
		...
		if err := readJSONFile(filepath.Join(run, "score.json"), &sc); err != nil {
			return nil, fmt.Errorf("%s: run `flat poc stats` first (%w)", run, err)
		}
		return &sc, nil
	}
	f, err := load(*flatRun)
