...
fail it. The fix is to ask and rule at package scope, and to teach `flat check` to flag a
...
- **The lead was the slow path.** Every request the lead ruled waited for its tick; every
  ...
  was correct and cost 4,614 output tokens to write twice; `flat digest` had the same content
  for free.
