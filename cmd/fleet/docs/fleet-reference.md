...
- **Five rules.** Location is identity (a directory's line in `roles.map` decides a
  ...
  or a machine resource), and a branch lease blocks only a holder active on it. The substrate knows no domain word. Done is evidence (a receipt
...
(`cmd/fleet/docs/OVERVIEW.md:6-11`, `cmd/fleet/README.md:36-39`).
...
hook records the launch directory (`cmd/fleet/internal/fleet/session.go:101-103`), and at
every SessionStart it resolves the role from it
(`cmd/fleet/internal/fleet/session.go:108-120`). A `compact` or `clear` SessionStart keeps
the recorded launch directory, because Claude Code sends the shell's current directory as
that event's cwd (`cmd/fleet/internal/fleet/session.go:169-182`). Two different match rules apply
...
(`cmd/fleet/internal/fleet/session.go:95-107`). The hook also refuses a Bash `cd` or
...
(`cmd/fleet/internal/fleet/session.go:55-143`, `cmd/fleet/internal/fleet/hook.go:565-572`).
...
| `sessions/<sid>.json` | hook (every event); the lease path writes a minimal one before a first lease | identity, launch dir, role, seat, lane, branch, pid and pid kind, turn state, last event, `last_writes` | `cmd/fleet/internal/fleet/session.go:55-143`, `cmd/fleet/internal/fleet/policy.go:534-542` |
| `leases/<key>.json` | hook (first write, branch switch, takeover, seat occupancy at start, release at end); verbs `take`, `drop`, `revoke` | one holder per key; after a takeover, its `takeover` record | `cmd/fleet/internal/fleet/policy.go:434`, `cmd/fleet/internal/fleet/policy.go:458-472`, `cmd/fleet/internal/fleet/session.go:681-683`, `cmd/fleet/internal/fleet/lease.go:127-240`, `cmd/fleet/internal/verbs/keys.go:117-231` |
...
| `last-word/<key>.json` | hook at Stop | the session's last assistant text on that branch | `cmd/fleet/internal/fleet/session.go:364-385` |
...
- A **branch** is leased implicitly, by writing to it, and a dead or idle holder's branch
  passes the same way, to the next writer. `fleet take` refuses branch keys
  (`cmd/fleet/internal/verbs/keys.go:118-120`); only the operator's `fleet revoke --to`
  hands a branch to a named session regardless of its holder (below).
...
- A **seat** is also a `slot:` key, but it is an occupancy lease the SessionStart hook
  ...
  (`cmd/fleet/internal/fleet/session.go:638-695`). It cannot be taken or dropped by hand
...
branch holder quiet on it), orphaned (dead resource holder), dead (dead branch holder).
Callers that act on its answer read it inside `KeyLock`
(`cmd/fleet/internal/fleet/policy.go:311-314`).

### Dead, alive, unreadable

`Liveness(sid)` returns two booleans, alive and known
(`cmd/fleet/internal/fleet/policy.go:353-376`):
...
- **Seat occupancy.** At SessionStart, `occupySlot` displaces the seat's previous occupant
  ...
  which `SessionAlive` treats as dead (`cmd/fleet/internal/fleet/session.go:669-683`,
  `cmd/fleet/internal/fleet/lease.go:252-255`). The code accepts this on purpose: a wrongly
  displaced seat costs a seat, not a collision (`cmd/fleet/internal/fleet/session.go:642-646`).
...
Liveness says a process answers; it does not say anyone is working. On 2026-09-13 three
Codex sessions that were out of usage for days held five lanes' branches: each record named
the Codex desktop app's pid, which kept answering, and one had its turn left open by the
...
- `LastActiveOn` is the latest of the lease's own `since`; the holder's `last_seen` for
  ...
  just admitted (`cmd/fleet/internal/fleet/session.go:136-167`,
...
- `turn_open` is not used: the out-of-usage session above had left it true.
- In-flight command records are not used either. Codex leaves a PreToolUse without its
  PostToolUse for about one command in twelve, so an `inflight/` record does not show that
  a command is still running.
- A resource is never idle. Its holder keeps it until `fleet drop`, because the machine it
  names can be busy with no session touching it.
...
- **Branch takeover** is automatic on the next write when the holder is known dead or idle
  on the branch (step 5 above). No operator command is involved. The lease's `takeover`
  record and the `takeovers` field of that evaluation's `events.jsonl` row say who took it,
  from whom, when and why; `fleet report` lists them (`cmd/fleet/internal/report/report.go`),
...
- **Revoke** is the operator's override: `fleet revoke <key> --to <session>` writes the
  ...
  (`cmd/fleet/internal/fleet/session.go:193-207`).
...
`cmd/fleet/model/CRASH-REPLACEMENT.md:90-103`). An idle holder is exposed the same way: a
tool call it was admitted to before it went quiet — a command still running past
`FLEET_IDLE_S`, or one waiting that long on a permission prompt — can still write after the
branch has changed hands. Resources taken with `fleet take` are safe
...
process the old one left may still be writing (`cmd/fleet/internal/fleet/session.go:691-693`).
...
- **The decision.** `action write(s)` abstracts `CheckLease` as one atomic step. In the
  ...
  refused (`cmd/fleet/model/model/reference.qnt:44-71`). The model has no idle state: the
  code's live holder that is idle on a branch is taken over like the model's `Dead`, and
  the model's `Alive` stands for a holder active on the key (`cmd/fleet/model/SOURCE_MAP.md`).
