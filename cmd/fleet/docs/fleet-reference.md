# Fleet runtime and model reference — September 2026 snapshot

This is the optional engineering reference from [PR #329](https://github.com/itsHabib/workbench/pull/329),
reviewed at `d82d65464de303abdebf15ece1095e78a10a83c1`. Its implementation citations and
numbered trace describe source snapshot `ea76b6230210d42802cb2967938341090e73dd3d`.
Read [Fleet 101](fleet-101.md) first for the current reader path. This reference is
for investigating the old implementation and its formal-model limits, not for copying
recovery commands into a current installation.

Ordinary shell writes such as `printf x > file.txt`, generators and scripts can pass
through the hook without a branch-lease check. Only recognized write admissions receive
that check; tools outside the matcher bypass the hook. Leases do not isolate files or stop
...
- #330 replaced the stdin-copy transport below with a private, exclusive
  `.request.json` file. Node receives its path in argv. The old stdin race is fixed;
  the historical store table, delivery step 11 and self-test 13 describe the old transport.
  See [provider.go](../internal/provider/provider.go).
- Command construction now includes request-file I/O. A construction failure can still
  strand a published `starting` record in [runtime.go](../internal/watch/runtime.go).
  This is a known reliability gap, not a recommended experiment prerequisite. Retain the
  reproducer and fix known pre-spawn errors; do not delete records or invent terminal evidence
  for an attempt that might have started. Runtime repair is tracked separately from this guide.
...
The following snapshot preserves source locations and historical traces. Factual clarifications
from both independent reviews are incorporated below; historical failure schedules remain
explicitly historical. It proves neither current platform acceptance nor learner effectiveness.
...
request carries the ownership row between them (`cmd/fleet/README.md:391-401`). Merge
...
(`cmd/fleet/internal/fleet/lanes.go:107-138`). At a `startup` or `resume` SessionStart the
hook records the launch directory (`cmd/fleet/internal/fleet/session.go:100-102`), and at
every SessionStart it resolves the role from it (`cmd/fleet/internal/fleet/session.go:107-119`).
A `compact` or `clear` SessionStart keeps the recorded launch directory, because Claude Code
sends the shell's current directory as that event's cwd
(`cmd/fleet/internal/fleet/session.go:138-151`). Two different match rules apply
...
(`cmd/fleet/internal/fleet/session.go:94-106`). The hook also refuses a Bash `cd` or
...
(`cmd/fleet/internal/fleet/session.go:55-135`, `cmd/fleet/internal/fleet/hook.go:505-512`).
...
| `sessions/<sid>.json` | hook (every event); the lease path writes a minimal one before a first lease | identity, launch dir, role, seat, lane, branch, pid and pid kind, turn state, last event, `last_writes` | `cmd/fleet/internal/fleet/session.go:55-135`, `cmd/fleet/internal/fleet/policy.go:488-496` |
| `leases/<key>.json` | hook (first write, branch switch, seat occupancy at start, release at end); verbs `take`, `drop`, `revoke` | one holder per key | `cmd/fleet/internal/fleet/policy.go:399`, `cmd/fleet/internal/fleet/session.go:650-652`, `cmd/fleet/internal/fleet/lease.go:127-240`, `cmd/fleet/internal/verbs/keys.go:117-231` |
...
| `last-word/<key>.json` | hook at Stop | the session's last assistant text on that branch | `cmd/fleet/internal/fleet/session.go:333-354` |
| `inflight/`, `locks/` | hook (Bash PreToolUse writes, PostToolUse and SessionEnd remove) | a running command's start time; a running expensive rule | `cmd/fleet/internal/fleet/hook.go:453-487`, `cmd/fleet/internal/fleet/lease.go:233-240` |
| `costs.jsonl`, `overrides.jsonl` | hook | measured command durations; accepted `FLEET_ALLOW_SLOW` overrides | `cmd/fleet/internal/fleet/hook.go:484`, `cmd/fleet/internal/fleet/policy.go:575` |
...
(`cmd/fleet/internal/fleet/hook.go:514-524`). The only deletion path is the reference
...
| PostToolUse | the in-flight record | `last_write`/`last_writes`, `costs.jsonl`, the `gh pr` cache (`cmd/fleet/internal/fleet/hook.go:462-499`) |
| Stop | the transcript | `turn_open: false`, `last_stop_at`, `last-word/` (`cmd/fleet/internal/fleet/hook.go:505-512`) |
| SessionEnd | the store's leases, locks, in-flight records and stop flags | `ended: true`; releases branch leases, cost locks, in-flight records and revoke flags in its favour (`cmd/fleet/internal/fleet/hook.go:514-524`, `cmd/fleet/internal/fleet/lease.go:223-240`) |
...
- A **seat** is also a `slot:` key, but it is an occupancy lease the SessionStart hook
  ...
  (`cmd/fleet/internal/fleet/session.go:607-664`). It cannot be taken or dropped by hand
...
- **Seat occupancy.** At SessionStart, `occupySlot` displaces the seat's previous occupant
  ...
  which `SessionAlive` treats as dead (`cmd/fleet/internal/fleet/session.go:638-652`,
  `cmd/fleet/internal/fleet/lease.go:252-255`). The code accepts this on purpose: a wrongly
  displaced seat costs a seat, not a collision (`cmd/fleet/internal/fleet/session.go:611-615`).
...
(`cmd/fleet/internal/fleet/hook.go:315-348`):
...
1. **The branch being written.** `preWriteVerdicts` checks, in order: a stop flag on this
   ...
   branch before being refused (`cmd/fleet/internal/fleet/hook.go:326-337`); then
   `CheckLease` on the target branch, but only when the call is a write
   (`cmd/fleet/internal/fleet/hook.go:338-342`). A call is a write when the tool is Edit,
...
2. **The branches a switch is headed for.** `switchDestinations` finds every branch a
   ...
   (`cmd/fleet/internal/fleet/hook.go:429-451`, `cmd/fleet/internal/fleet/policy.go:600-637`).
...
- **Revoke** is the operator's override: `fleet revoke <key> --to <session>` writes the
  ...
  (`cmd/fleet/internal/fleet/session.go:162-176`).
...
process the old one left may still be writing (`cmd/fleet/internal/fleet/session.go:660-662`).
...
the latest matching dispatch's `for` role, otherwise configured `LATE_TO`
(`cmd/fleet/internal/watch/late.go:272-293`). Org's parent is not consulted; with no
recipient the watcher records `late-no-recipient`. The message id is derived
...
| 29 | hook (Stop, SessionEnd) | `turn_open: false`, `last-word/`; at SessionEnd `ended: true` and branch leases released | `cmd/fleet/internal/fleet/hook.go:505-524` |
...
`verified` 2026-09-12 at `ea76b62`; the following schedule describes that old revision.
#330 fixed the transport; see the changes-since-snapshot section above.
...
- **Windows.** Install, hook projection and the `cd` guard run in CI on `windows-latest`
  ...
  is `parent-unverified` (section 5). #324, the Windows hook-validation follow-up after the executable packaging fix, is open
  (`live`).
...
7. The bridge exited 0 but `provider_terminal` is false. Is the directory free? *(Not enough information.
   Exit alone is insufficient; matching never-started evidence or the supported bounded
   quiescence proof can also release it. Otherwise it remains reserved.)*
...
| "The watcher writes only under `watch/` plus those stamps on mail records"; "That is the whole of what it writes outside watch/" | `cmd/fleet/README.md:291`, `cmd/fleet/internal/watch/watch.go:20-21` | It also publishes lateness reports as new mail records (`cmd/fleet/internal/watch/late.go:109-120`), writes `cache/github/` every tenth tick (`cmd/fleet/internal/watch/watch.go:153-166`), and appends to `hook-errors.jsonl` (`cmd/fleet/internal/watch/watch.go:636`) |
| "The command launcher is a transitional interface; durable Claude/Codex session integration is the next runtime step" | `cmd/fleet/README.md:458-460` | Behind code since #318: the watcher runs provider sessions through the bridge and resumes them (`cmd/fleet/internal/watch/runtime.go:97-213`); `FOLLOWUPS.md:699-700` records it as a known stale paragraph |
