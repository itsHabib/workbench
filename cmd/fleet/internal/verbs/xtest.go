...
func xtest(verb string, a []string) error {
	...
	if ok, err := xtestLeases(verb, arg); ok {
		return err
	}
	if ok, err := xtestChecks(verb, a, arg); ok {
		return err
	}
	return exitCode(2, "fleet: unknown test-support verb "+verb)
}

// xtestLeases is the lease protocol as verbs, for the suite's facade.
func xtestLeases(verb string, arg func(int) string) (bool, error) {
	switch verb {
	case "x-lease":
	...
	case "x-hold":
		...
		if err != nil {
			return true, refuse("usage: fleet x-hold <key> <seconds>")
		}
		return true, holdKey(arg(0), secs)
	case "x-stop-flag":
...
// xtestChecks is the read-only checks as verbs, for the suite's facade.
func xtestChecks(verb string, a []string, arg func(int) string) (bool, error) {
	switch verb {
	case "x-id":
...
// holdKey holds a key's lock for secs seconds, printing `held` once it is taken so a
// caller can synchronise on it. Killed by the caller when the window should close.
func holdKey(key string, secs float64) error {
