...
never auto-allow an unobservable approval capability; post-tool evidence can
never grant authority.

See [`docs/hooks.md`](docs/hooks.md) for the response table, audit path, direct
offline fixtures, and honest hook-failure matrix.

## Profile projection

`codexguard projection status` creates a non-mutating plan for the reviewed
absolute-path hook definition and restrictive rules asset. `projection apply`
publishes only the exact hash-bound plan and refuses divergent or racing
targets. The hook command is bound to the resolved absolute path of the
`codexguard` executable running projection, so repository-local executable
lookup is never trusted. Projection does not trust the hook or activate a Codex
profile.

See [`docs/install.md`](docs/install.md) for the temporary-home demo and the
separate operator activation steps.
