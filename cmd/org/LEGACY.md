# Legacy Baton runtime

The journal protocol below is preserved for existing records and migration only.
Prefix old command examples with `org legacy` (for example, `org legacy boot`).
New roles use Markdown cards; see [README.md](README.md).
The default Stop hook no longer writes chain marks.
