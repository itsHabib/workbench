...
const (
	...
	// the effect (the judgment + re-reduction + the resulting action) is recorded
	// by the existing judge mechanism; this stamp only names who closed the loop
	// and links the judgment, parented to the escalation it resolves. It sits
	// outside the action/escalation outcome families on purpose: the cycle count
	// and the parked/ready projections ignore it, so recording a resolution never
	// counts as a review cycle or masquerades as a fresh park.
	//
	// Asymmetry to close before a projection reads this: a hand-run `gate judge`
	// closes the identical loop WITHOUT a stamp, so "who closed this park?" is
	// answerable from the log for the back-channel path only. Extending the stamp
	// to the judge path (or teaching a reader that its absence means "resolved via
	// judge") is a named follow-up in the plane spec — not built here.
